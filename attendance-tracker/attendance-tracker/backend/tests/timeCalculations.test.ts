...
describe('Time Calculations', () => {
  describe('parseTime', () => {
    it('should parse valid time strings correctly', () => {
      const result1 = parseTime('09:00');
      const result2 = parseTime('17:30');
      const result3 = parseTime('00:00');
      
      expect(result1).toBeInstanceOf(Date);
      expect(result2).toBeInstanceOf(Date);
      expect(result3).toBeInstanceOf(Date);
      
      if (result1) expect(result1.getHours()).toBe(9);
      if (result2) expect(result2.getHours()).toBe(17);
      if (result2) expect(result2.getMinutes()).toBe(30);
      if (result3) expect(result3.getHours()).toBe(0);
    });

    it('should handle invalid time strings', () => {
      expect(parseTime('')).toBeNull();
      const invalidResult = parseTime('invalid');
      if (invalidResult) {
  ...
  describe('calculateWeeklyTotals', () => {
    it('should calculate weekly totals correctly', () => {
      ...
      const result = calculateWeeklyTotals(dailyRecords);
  ...
  describe('calculatePay', () => {
    it('should calculate pay with regular hours only', () => {
      ...
      const result = calculatePay(timeData, rates);
